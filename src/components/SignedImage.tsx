...
export function Avatar({ path, name, size = 40, onClick, className }: {
  path: string | null | undefined;
  name?: string;
  size?: number;
  onClick?: () => void;
  ...
  const base = "inline-flex items-center justify-center rounded-full overflow-hidden bg-[var(--brand-soft)] text-[var(--brand)] font-semibold shrink-0 " + (className ?? "");
  if (path) {
    return (
